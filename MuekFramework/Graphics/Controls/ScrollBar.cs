        ...
        OnInput += e =>
        {
            if (e.Type == (uint)SDL.EventType.MouseWheel)
            {
                if (e.Wheel.Y > 0)
                {
                    ...
                    Console.WriteLine("Mouse wheel up " + $"ScrollY={_scrollY}");
                }

                if (e.Wheel.Y < 0)
                {
                    ScrollY -= e.Wheel.Y * ScrollSpeedY;
                    Console.WriteLine("Mouse wheel down " + $"ScrollY={_scrollY}");
                }
                if (e.Wheel.X > 0)
                {
                    ScrollX += e.Wheel.X  * ScrollSpeedX;
                    Console.WriteLine("Mouse wheel right " + $"ScrollX={_scrollX}");
                }

                if (e.Wheel.X < 0)
                {
                    ScrollX += e.Wheel.X  * ScrollSpeedX;
                    Console.WriteLine("Mouse wheel left " + $"ScrollX={_scrollX}");
                }
            }
        };
        OnRender += c =>
